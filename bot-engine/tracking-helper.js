// ═══════════════════════════════════════════════════════════════
// BotForge — Fast-Path Tracking Engine for Telegram (ESM)
...
export function customerStatusLabel(status, businessType) {
...
export const TRACKING_INTENT_REGEX = /(تتبع|وين راه|وين وصل|وقتاش يوصل|حالة الطلب|رقم الطلب|كود التتبع|livraison|suivi)/i;
export const TRACKING_CODE_REGEX = /#?DZ-[A-Za-z0-9]{4,10}/i;
...
export function formatSingleOrderCard(order, businessType) {
  const voice = voiceFor(businessType);
...
export function formatMultipleOrdersList(orders, businessType) {
  const voice = voiceFor(businessType);
...
export function formatNoOrdersFound(searchedCode = null) {
  if (searchedCode) {
    return `🔍 لم نجد أي طلبية مسجلة برقم التتبع (\`#${searchedCode}\`).\nيرجى التأكد من كتابة الكود بشكل صحيح أو التواصل مع المتجر مباشرة للمساعدة.`;
  }
  return `🔍 لم نجد أي طلبات مسجلة لحسابك حالياً.\nإذا كنت قد قمت بالطلب مؤخراً، أرسل لنا كود التتبع المذكور في رسالة تأكيد الطلب وسنساعدك فوراً!`;
}
