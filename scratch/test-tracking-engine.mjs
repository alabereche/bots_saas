// ═══════════════════════════════════════════════════════════════
// Automated Unit Tests for Modular Tracking & Fast Path Engine (ESM)
// ═══════════════════════════════════════════════════════════════
...
import {
  ...
  PROVIDER_NAMES,
} from '../bot-engine/tracking-helper.js';
...
positiveCases.forEach(phrase => {
  const isIntent = isTrackingIntent(phrase);
...
assert.strictEqual(extractTrackingCode('تتبع DZ-K7M4Q9 خويا'), 'DZ-K7M4Q9');
assert.strictEqual(extractTrackingCode('DZ-223456'), 'DZ-223456');
assert.strictEqual(extractTrackingCode('ما عنديش كود'), null);
...
const card = formatSingleOrderCard(sampleOrder);
...
const multiList = formatMultipleOrdersList(multiOrders);
