  ...
  try {
    ...
    if (order) {
      firestore.saveOrder({
        ...
        if (order.product) {
          firestore.adjustProductStock(config.id, order.product, 'reserve', config.userId).catch(() => {});
        }

        // Tracking code follow-up: the code is generated by saveOrder AFTER
        // the AI confirmation went out, so it arrives as its own short
        // message. WhatsApp renders triple-backtick blocks with a built-in
        // tap-to-copy chip — no design change, one-tap copy for free.
        if (saved.trackingCode && !saved.isUpdate) {
          const trackingMsg = 'رمز تتبع طلبك:\n```' + saved.trackingCode + '```\nاكتب «تتبع» في أي وقت للاستعلام عن حالة طلبيتك.';
          msg.client.sendText(userId, trackingMsg).catch(() => {});
        }
